...
热装的插件行只活在内存里，如果之后**手动编辑** profile 的 `cordis.patch.yml`，HMR 会按启动时的快照全量重组插件树，热装的行会被卸掉（重启后恢复，因为清单里还有这些包）。同理，官方"设置 → 插件列表"显示的是启动时的快照，热装的包要等重启才出现在那里。这两个都属于设计内行为：本插件只管清单的增删，不与补丁层抢地盘。
