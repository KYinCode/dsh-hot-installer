...
| --- | --- | --- |
| `0.1.5-rc.2`（npm `latest`，普通用户默认装到的） | HMR 只监听补丁文件，**完全不看 profile 清单**：装/卸/升级都必须重启才生效 | **全部能力**——装、卸、升级三种操作都免重启 |
| `0.1.6-alpha.2` 及以后 | 平台自带 `dsh-hmr` 接管清单，**装/卸已原生免重启** | 补平台跳过的部分：**依赖版本升级**（`add pkg@新版本`）仍免重启；另带预检、失败回滚、紧急卸载保护链 |
